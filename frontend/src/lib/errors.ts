export function friendlyError(e: unknown): string {
  ...
  if (/TargetNotReached/.test(msg)) return 'The target has not been reached yet.'
  if (/NotRefundable/.test(msg)) return 'Refunds open only after the deadline if the target was missed.'
